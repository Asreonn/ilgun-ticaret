...
export default function Admin() {
  ...
  const edit = (p: Product) => setForm({
    ...
    images: [...(p.product_images || [])].sort((a,b) => a.sort_order-b.sort_order).map((i) => [i.image_path, i.alt_text, i.source_image_url].filter((part) => part != null && part !== "").join("|")).join("\n")
  });
  ...
  const save = async (event: FormEvent) => {
    ...
    const images = form.images.split("\n").map((line) => line.trim()).filter(Boolean).map((line, i) => { const [image_path, alt_text, source_image_url] = line.split("|"); return { product_id: productId, image_path: image_path.trim(), alt_text: (alt_text || form.name).trim(), source_image_url: source_image_url?.trim() || null, sort_order: i }; });
