...
export default function Home() {
  ...
  const [previousHeroProduct, setPreviousHeroProduct] = useState<Product | null>(null);
  ...
  const showHero = (next: number) => {
    ...
    setPreviousHeroProduct(heroProduct || null);
    setHeroDirection(next < heroIndex ? "previous" : "next");
    setHeroIndex(normalized);
  };
  useEffect(() => {
    if (!previousHeroProduct) return;
    const timer = window.setTimeout(() => setPreviousHeroProduct(null), 920);
    return () => window.clearTimeout(timer);
  }, [heroIndex, previousHeroProduct]);
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [heroIndex, heroPaused, heroProduct, heroProducts.length]);
  return <>
    ...
    <section className="hero"><div className="container hero-grid"><div className="hero-copy"><span className="pill"><Sparkles size={16} /> Seçili teknoloji ürünleri</span><h1>Teknoloji <em>seçkisini</em> keşfedin</h1><p>Ses, aksesuar, kişisel bakım ve küçük ev aletleri.</p><div className="hero-actions"><Link className="button primary" to="/products">Ürünleri İncele <ArrowRight size={18} /></Link><a className="button secondary" href={whatsappUrl()} target="_blank" rel="noreferrer"><MessageCircle size={18} /> WhatsApp</a></div></div>{heroProduct && <div className={`hero-carousel ${heroPaused ? "paused" : ""}`} data-direction={heroDirection} onMouseEnter={() => setHeroPaused(true)} onMouseLeave={() => setHeroPaused(false)} onFocusCapture={() => setHeroPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setHeroPaused(false); }} onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={(event) => { const distance = event.changedTouches[0].clientX - touchStart.current; if (Math.abs(distance) > 45) showHero(heroIndex + (distance < 0 ? 1 : -1)); }}><div className="hero-visual"><div className="hero-orbit"></div><span className="hero-transition-glow" key={`glow-${heroProduct.id}`} aria-hidden="true"/>{previousHeroProduct && <Link className="hero-product-stage hero-product-exit" key={`previous-${previousHeroProduct.id}`} to={`/products/${previousHeroProduct.slug}`} tabIndex={-1} aria-hidden="true"><ImageWithLoader src={imageUrl(previousHeroProduct.main_image)} alt="" width="720" height="720" /></Link>}<Link className="hero-product-stage hero-product-enter" key={heroProduct.id} to={`/products/${heroProduct.slug}`} aria-label={`${heroProduct.name} ürününü incele`}><ImageWithLoader src={imageUrl(heroProduct.main_image)} alt={heroProduct.name} width="720" height="720" loading="eager" /></Link><div className="floating-note" key={`note-${heroProduct.id}`} aria-live="polite"><span>{heroProduct.brand || "İLGÜN SEÇKİSİ"} · {heroProduct.model}</span><strong>{heroProduct.name}</strong></div></div><div className="hero-slider-controls"><button onClick={() => showHero(heroIndex - 1)} aria-label="Önceki ürün"><ChevronLeft/></button><div>{heroProducts.map((product, index) => <button key={product.id} className={index === heroIndex ? "active" : ""} onClick={() => showHero(index)} aria-label={`${index + 1}. ürünü göster`}/>)}</div><button onClick={() => showHero(heroIndex + 1)} aria-label="Sonraki ürün"><ChevronRight/></button></div></div>}</div></section>
