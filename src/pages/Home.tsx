...
export default function Home() {
  ...
  return <>
    ...
    <section className="hero"><div className="container hero-grid"><div className="hero-copy"><span className="pill"><Sparkles size={16} /> Seçili teknoloji ürünleri</span><h1>Günlük Hayatı <em>Kolaylaştıran</em> Teknoloji</h1><p>Ses, aksesuar, kişisel bakım ve küçük ev aletleri.</p><div className="hero-actions"><Link className="button primary" to="/products">Ürünleri İncele <ArrowRight size={18} /></Link><a className="button secondary" href={whatsappUrl()} target="_blank" rel="noreferrer"><MessageCircle size={18} /> WhatsApp</a></div></div>{heroProduct && <div className="hero-carousel" onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={(event) => { const distance = event.changedTouches[0].clientX - touchStart.current; if (Math.abs(distance) > 45) showHero(heroIndex + (distance < 0 ? 1 : -1)); }}><div className="hero-visual"><div className="hero-orbit"></div><Link className="hero-product-stage" key={heroProduct.id} to={`/products/${heroProduct.slug}`} aria-label={`${heroProduct.name} ürününü incele`}><img src={imageUrl(heroProduct.main_image)} alt={heroProduct.name} width="720" height="720" /></Link><div className="floating-note"><span>{heroProduct.brand || "İLGÜN SEÇKİSİ"} · {heroProduct.model}</span><strong>{heroProduct.name}</strong></div></div><div className="hero-slider-controls"><button onClick={() => showHero(heroIndex - 1)} aria-label="Önceki ürün"><ChevronLeft/></button><div>{heroProducts.map((product, index) => <button key={product.id} className={index === heroIndex ? "active" : ""} onClick={() => showHero(index)} aria-label={`${index + 1}. ürünü göster`}/>)}</div><button onClick={() => showHero(heroIndex + 1)} aria-label="Sonraki ürün"><ChevronRight/></button></div></div>}</div></section>
