...
export default function ProductDetail() {
  ...
  return <>
    ...
    <div className="container product-page"><nav className="breadcrumbs" aria-label="Sayfa yolu"><Link to="/">Ana Sayfa</Link><ChevronRight size={14}/><Link to="/products">Ürünler</Link><ChevronRight size={14}/><span>{product.model}</span></nav>
      <div className="detail-grid"><Reveal className="gallery" direction="left"><ProductSlider className="gallery-slider" items={images} getKey={(img) => img.image_path} index={selected} onIndexChange={setSelected} variant="page" previousLabel="Önceki görsel" nextLabel="Sonraki görsel" dotLabel={(index) => `${index + 1}. görseli aç`} renderSlide={(img) => <div className="main-image"><ImageWithLoader src={imageUrl(img.image_path || product.main_image)} alt={img.alt_text || product.name} width="900" height="900" loading="eager" /></div>} /><div className="thumbnails">{images.map((img, i) => <button key={`${img.image_path}-${i}`} className={selected === i ? "active" : ""} onClick={() => setSelected(i)} aria-label={`${i + 1}. görseli aç`}><ImageWithLoader src={imageUrl(img.image_path)} alt="" loading="lazy" /></button>)}</div></Reveal>
        <Reveal className="detail-info" direction="right"><span className="eyebrow">{product.brand || "ÜRÜN"} · {product.model}</span><h1>{product.name}</h1><a className="detail-rating" href="#reviews"><span className="rating-stars" aria-hidden="true">{Array.from({ length: 5 }).map((_, index) => <Star key={index} className={index < Math.round(product.rating_average || 0) ? "filled" : ""}/>)}</span><strong>{product.review_count ? product.rating_average?.toFixed(1) : "Henüz puanlanmadı"}</strong><span>{product.review_count || 0} yorum</span></a><p className="lead">{product.short_description}</p><div className="price-block"><strong>{formatPrice(product.price, product.currency)}</strong>{product.old_price != null && <del>{formatPrice(product.old_price, product.currency)}</del>}<span className={`stock ${product.stock_status}`}><PackageCheck size={17} /> {stockText[product.stock_status]}</span></div>{product.price_checked_at && <p className="price-meta">{product.price_note?.startsWith("Piyasa emsali") ? "Piyasa emsali fiyat" : "Referans fiyat"} · {new Intl.DateTimeFormat("tr-TR", { dateStyle: "long" }).format(new Date(product.price_checked_at))} tarihinde kontrol edildi</p>}<ul className="quick-features">{product.product_features.slice(0, 5).map((f, i) => <li key={`${f.label}-${i}`}><Check size={17} /><span>{f.label}{f.value ? `: ${f.value}` : ""}</span></li>)}</ul><div className="detail-actions"><button className={`button primary ${added ? "added" : ""}`} onClick={addToCart}>{added ? <Check size={20}/> : <ShoppingBag size={20}/>} {added ? "Sepete Eklendi" : "Sepete Ekle"}</button><a className="button whatsapp order-button" href={whatsappUrl(product.name)} target="_blank" rel="noreferrer"><MessageCircle size={21} /> {product.price == null ? "Güncel Teklif Al" : "WhatsApp'tan Sipariş Ver"}</a></div><div className="direct-contact"><ShieldCheck size={19}/><span><strong>{site.contact}</strong> ile doğrudan görüşün · {site.phoneDisplay}</span></div></Reveal></div>
      <Reveal as="section" className="description-panel" direction="up"><div><span className="eyebrow">ÜRÜN HAKKINDA</span><h2>Detaylar</h2><p>{product.description}</p></div><div><h2>Teknik özellikler</h2><dl>{product.product_features.map((f, i) => <div key={`${f.label}-${i}`}><dt>{f.label}</dt><dd>{f.value || "Var"}</dd></div>)}</dl></div></Reveal>
      <ProductReviews productId={product.id} productSlug={product.slug}/>
      {related.length > 0 && <Reveal as="section" className="related section" direction="up"><div className="section-head"><h2>Benzer ürünler</h2><Link to={`/products/category/${product.category?.slug}`}>Kategoriyi gör</Link></div><ProductSlider className="related-slider" items={related} getKey={(item) => item.id} variant="peek" autoPlayMs={5000} previousLabel="Önceki benzer ürün" nextLabel="Sonraki benzer ürün" renderSlide={(item) => <ProductCard product={item} />} /></Reveal>}
    </div>
