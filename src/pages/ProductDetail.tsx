...
export default function ProductDetail() {
  ...
  return <>
    ...
    <div className="container product-page"><nav className="breadcrumbs" aria-label="Sayfa yolu"><Link to="/">Ana Sayfa</Link><ChevronRight size={14}/><Link to="/products">Ürünler</Link><ChevronRight size={14}/><span>{product.model}</span></nav>
      <div className="detail-grid"><Reveal className="gallery" direction="left"><div className="main-image"><ImageWithLoader key={images[selected]?.image_path} src={imageUrl(images[selected]?.image_path || product.main_image)} alt={images[selected]?.alt_text || product.name} width="900" height="900" loading="eager" /></div><div className="thumbnails">{images.map((img, i) => <button key={`${img.image_path}-${i}`} className={selected === i ? "active" : ""} onClick={() => setSelected(i)} aria-label={`${i + 1}. görseli aç`}><ImageWithLoader src={imageUrl(img.image_path)} alt="" loading="lazy" /></button>)}</div></Reveal>
      ...
      {related.length > 0 && <Reveal as="section" className="related section" direction="up"><div className="section-head"><h2>Benzer ürünler</h2><Link to={`/products/category/${product.category?.slug}`}>Kategoriyi gör</Link></div><div className="product-grid reveal-stagger">{related.map((p) => <ProductCard key={p.id} product={p} />)}</div></Reveal>}
    </div>
